...
param(
    [Parameter()]
    [string]$Repository = "Chris-Wolfgang/DbContextBuilder"
)
...
if ($Repository -eq "Chris-Wolfgang/DbContextBuilder" -or -not $Repository) {
    ...
    try {
        ...
        if ($Repository -eq "Chris-Wolfgang/DbContextBuilder") {
